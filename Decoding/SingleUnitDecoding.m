...
Bin_tr_Rate(:,3)=smooth1D(Bin_tr_Rate(:,3),smoothingRange);
...
for iter = 1:size(Shuffled_trial,1)
    ...
    % train and test
    % select 70% of the trials for training the model and 20% for test
...
subplot(1,3,3)
M=mean(rank_confusion_matrix,3)';
size(M)
figure
imagesc(matnorm(M,2))
axis xy
...
subplot(1,5,4)
plot(1:size(Cltraining,2),(Cltraining),'k','linewidth',2)
hold on
plot(1:100,mean(Cltraining),'r','linewidth',2)
xlabel('cl.labels')
ylabel('cl.templates')
